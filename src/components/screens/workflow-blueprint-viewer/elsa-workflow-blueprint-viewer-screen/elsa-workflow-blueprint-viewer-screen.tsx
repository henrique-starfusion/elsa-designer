...
export class ElsaWorkflowBlueprintViewerScreen {
  ...
  @Prop({ attribute: 'workflow-blueprint', reflect: true }) workflowBlueprintJson: string;
  ...
  @Watch('workflowBlueprintJson')
  async workflowBlueprintJsonChangedHandler(newValue: string) {
    let workflowBlueprint: WorkflowBlueprint = newValue ? JSON.parse(newValue) : {
      id: null,
      version: 1,
      activities: [],
      connections: [],
  ...
  async componentWillLoad() {
    await this.workflowBlueprintJsonChangedHandler(this.workflowBlueprintJson);
  }
  ...
  async loadActivityDescriptors() {
    state.activityDescriptors = JSON.parse(this.activityDescriptorsJson);
  }
...
Tunnel.injectProps(ElsaWorkflowBlueprintViewerScreen, ['culture']);

...
export class ElsaWorkflowDefinitionEditorScreen {
  ...
  @Event({
    eventName: 'workflow-definition-changed',
    bubbles: true,
    composed: true,
    cancelable: true
  }) workflowDefinitionChanged: EventEmitter<WorkflowDefinition>;
  @Prop({ attribute: 'activity-definitions', reflect: true }) activityDefinitionsJson: string;
  @Prop({ attribute: 'workflow-definition', reflect: true }) workflowDefinitionJson: string;
  ...
  async exportWorkflow() {
    const workflowDefinition = this.workflowDefinition || JSON.parse(this.workflowDefinitionJson);
    ...
    console.debug('exportWorkflow', workflowDefinition, versionOptions);
  }
  ...
  async importWorkflow(file: File) {
    ...
    try {
      this.workflowDefinition = this.workflowDefinition || JSON.parse(this.workflowDefinitionJson);
      this.workflowModel = this.mapWorkflowModel(this.workflowDefinition);
      this.importing = false;
      this.imported = true;
      setTimeout(() => this.imported = false, 500);
      await eventBus.emit(EventTypes.WorkflowImported, this, this.workflowDefinition);
    } catch (e) {
      console.error('importWorkflow', e);
      this.importing = false;
      this.imported = false;
  ...
  @Watch('activityDefinitionsJson')
  async activityDefinitionsJsonChangedHandler(newValue: string) {
    try {
      let activityDefinitions: Array<ActivityDefinition> = newValue ? JSON.parse(newValue) : [];
      this.updateActivityDefinitions(activityDefinitions);
      await this.loadActivityDescriptors();
      // await this.loadWorkflowStorageDescriptors();
    } catch (e) {
      console.error('activityDefinitionsJsonChangedHandler', e, newValue);
    }
  }

  @Watch('workflowDefinitionJson')
  async workflowDefinitionJsonChangedHandler(newValue: string) {
    try {
      let workflowDefinition: WorkflowDefinition = newValue ? JSON.parse(newValue) : ElsaWorkflowDefinitionEditorScreen.createWorkflowDefinition();
      this.updateWorkflowDefinition(workflowDefinition);
    } catch (e) {
      console.error('workflowDefinitionJson', e, newValue);
    }
  }
  ...
  @Listen('workflow-model-changed')
  async workflowModelChangedHandler(event: CustomEvent<WorkflowModel>) {
    const workflowModel = event.detail;
    await this.saveWorkflowInternal(workflowModel);
  }

  @Listen('workflow-definition-changed')
  workflowDefinitionChangedHandler(event: CustomEvent<WorkflowDefinition>) {
    eventBus.emit(EventTypes.WorkflowDefinitionChanged, this, event.detail);
  }
  ...
  async componentWillLoad() {
    // console.debug('componentWillLoad');
    ...
    await this.activityDefinitionsJsonChangedHandler(this.activityDefinitionsJson);
    await this.workflowDefinitionJsonChangedHandler(this.workflowDefinitionJson);
  ...
  async loadActivityDescriptors() {
    state.activityDescriptors = this.activityDefinitions || JSON.parse(this.activityDefinitionsJson);
  }
  ...
  async tryUpdateActivityInformation(activityId: string) {
    ...
    console.debug('tryUpdateActivityInformation', activityId);
    // this.workflowDefinition.id
    // this.activityStats = ;
    // if (!this.workflowInstance || this.workflowInstance.id !== this.workflowDefinition.id)
    //   this.workflowInstance = await client.workflowInstancesApi.get(this.workflowInstanceId);
  }

  updateActivityDefinitions(value: Array<ActivityDefinition>) {
  ...
  async saveWorkflowInternal(workflowModel?: WorkflowModel, publish?: boolean) {
    ...
    let workflowDefinition: WorkflowDefinition = {
      ...
      versionId: this.workflowDefinition.versionId,
      definitionId: this.workflowDefinition.definitionId || uuidv4(),
      contextOptions: this.workflowDefinition.contextOptions,
      deleteCompletedInstances: this.workflowDefinition.deleteCompletedInstances,
      description: this.workflowDefinition.description,
      displayName: this.workflowDefinition.displayName,
      isSingleton: this.workflowDefinition.isSingleton,
      name: this.workflowDefinition.name,
      tag: this.workflowDefinition.tag,
      channel: this.workflowDefinition.channel,
      persistenceBehavior: this.workflowDefinition.persistenceBehavior,
    ...
    const isNew = typeof workflowDefinition.id === 'undefined' && typeof workflowDefinition.definitionId === 'undefined' && typeof this.workflowDefinitionJson === 'undefined';

    if (isNew) {
      workflowDefinition.id = workflowDefinition.versionId = uuidv4();
    }
    ...
    try {
      console.debug('Saving workflow...', workflowDefinition);
      ...
      this.workflowDefinitionChanged.emit(workflowDefinition);
    } catch (e) {
      console.error(`saveWorkflowInternal`, e);
      this.saving = false;
      this.saved = false;
      this.networkError = e.message;
      setTimeout(() => this.networkError = null, 10000);
  ...
  async unpublishWorkflow() {
    console.debug('unpublishWorkflow', this.workflowDefinition);
    this.unPublishing = true;
    ...
    try {
      ...
      console.error(`unpublishWorkflow`, e);
      this.unPublishing = false;
  ...
  onTestActivityMessageReceived = async args => {
    ...
    console.debug('onTestActivityMessageReceived', args);
    ...
    return (
      <span>
        <button type="button"
          ...
          class="workflow-settings-button elsa-absolute elsa-top-15 elsa-right-28 elsa-inline-flex elsa-items-center elsa-p-2 elsa-rounded-full elsa-border elsa-border-transparent elsa-bg-white shadow elsa-text-gray-400 hover:elsa-text-blue-500 focus:elsa-text-blue-500 hover:elsa-ring-2 hover:elsa-ring-offset-2 hover:elsa-ring-blue-500 focus:elsa-outline-none focus:elsa-ring-2 focus:elsa-ring-offset-2 focus:elsa-ring-blue-500">
  ...
  public static createWorkflowDefinition(): WorkflowDefinition {
    return {
      definitionId: null,
      version: 1,
      activities: [],
...
DashboardTunnel.injectProps(ElsaWorkflowDefinitionEditorScreen, ['culture', 'monacoLibPath', 'basePath', 'serverFeatures']);
